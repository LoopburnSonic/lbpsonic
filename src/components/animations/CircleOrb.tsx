...
const CircleOrb: FC<CircleOrbProps> = ({
  hue = 240, // Default to blue hue for LoopFuse
  hoverIntensity = 0.5,
  rotateOnHover = true,
  ...
  const frag = /* glsl */ `
    ...
    const vec3 baseColor1 = vec3(0.0, 0.5, 1.0);  // Bright Blue
    const vec3 baseColor2 = vec3(0.4, 0.0, 0.8);  // Dark Purple
    const vec3 baseColor3 = vec3(0.5, 0.0, 1.0);  // Slightly lighter Purple
    ...
    float circleDistance(vec2 uv) {
      // Simple circle distance - much smaller size
    ...
    vec4 draw(vec2 uv) {
      ...
      float circleDist = circleDistance(uv);

      // Static thickness for stable outer shape (very thin/minimal)
      float thickness = 0.06;

      // Create the static circle shape
      float v0 = light1(1.0, 10.0, circleDist);
      v0 *= smoothstep(thickness + 0.05, thickness, circleDist);

      // Single flame particle moving along the circle
      float a = iTime * -1.0;
      vec2 flamePos = vec2(cos(a), sin(a)) * 0.5;

      float flameDistance = distance(uv, flamePos);
      float flameIntensity = light2(1.5, 4.0, flameDistance);
      flameIntensity *= light1(1.0, 50.0, circleDist); // Keep flame inside the shape
      ...
      float v2 = fadeOut;
      float v3 = smoothstep(thickness + 0.1, thickness - 0.05, circleDist);
